...
host = "http://oss.wuhu-a.brainpp.cn"
...
class CTDetDataset(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    img_dir = 'coco/images/trainval2017/'
    img_path = os.path.join(img_dir, file_name)
    ann_ids = self.coco.getAnnIds(imgIds=[img_id])
    anns = self.coco.loadAnns(ids=ann_ids)
    num_objs = min(len(anns), self.max_objs)

    img_obj = s3_client.get_object(Bucket="wangtiancai", Key=img_path)
    img = np.array(Image.open(io.BytesIO(img_obj['Body'].read())).convert('RGB'), dtype=np.float32)
        ...
        if ann['category_id'] == 1:
          ...
          if ann['obj_bbox'] != []:
            for i, obbox in enumerate(ann['obj_bbox']):
              o_bbox = self._coco_box_to_bbox(obbox[:4])
              o_act = obbox[4:]
              ...
              if o_h > 0 and o_w > 0:
                ...
                for cls_id in o_act:
                    draw_gaussian(hm_act[cls_id], act_ct_int, radius)
